...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function SetupProgpowWrapper() {
        emitLog('Starting '+__dirname+'/../wrapper/progpow_ethash_wrapper '+global.progpow_wrapper_server);
        var progpow_wrapper = spawn(__dirname+'/../wrapper/progpow_ethash_wrapper', [global.progpow_wrapper_server, "true"]); // change last parameter to "false" to enable miner mixhash checking
        if (progpow_wrapper == null) {
            emitErrorLog("ProgPoW Wrapper: progpow_wrapper == null");
        } else {
            progpow_wrapper.on('error', function(error) {
                emitErrorLog("ProgPoW Wrapper: "+error.toString().trim());
            });
            
            progpow_wrapper.stdout.on('data', function(data) {
                emitLog("ProgPoW Wrapper: "+data.toString().trim());
            });
            
            progpow_wrapper.stderr.on('data', function(data) {
                emitErrorLog("ProgPoW Wrapper: "+data.toString().trim());
            });
        }
    }
